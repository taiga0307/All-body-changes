class Customers::HomesController < ApplicationController
  ...
  def top
    ...
    @gyms = Gym.find(GymFavorite.group(:gym_id).order('count(gym_id) desc').limit(8).pluck(:gym_id))
    @products = Product.find(ProductFavorite.group(:product_id).order('count(product_id) desc').limit(8).pluck(:product_id))
